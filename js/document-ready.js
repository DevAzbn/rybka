...
$(".arrow-slider").each(function(i){var e=$(this),r=e.find(".img-block .item"),t=(e.find(".text-content"),e.find(".title-block"),e.find(".arrow-block")),a=t.find(".point-line");r.each(function(i){$("<a/>",{"class":"item",html:'<span class="point" ></span>',href:"#image-"+i}).on("click.arrow-slider.point",function(i){console.log("click.arrow-slider.point");var e=$(this).index();a.find(".item").removeClass("active"),r.hide().removeClass("active"),$(this).addClass("active"),r.eq(e).fadeIn("fast").addClass("active")}).appendTo(a)}),t.find(".btn-arrow.right").on("click.arrow-slider.right",function(i){var e=a.find(".item"),r=e.filter(".active").eq(0).index(),t=e.eq(r).next(".item");t.size()?t.trigger("click.arrow-slider"):e.eq(0).trigger("click.arrow-slider")}),t.find(".btn-arrow.left").on("click.arrow-slider.left",function(i){var e=a.find(".item"),r=e.filter(".active").eq(0).index(),t=e.eq(r).prev(".item");t.size()?t.trigger("click.arrow-slider"):e.eq(-1).trigger("click.arrow-slider")}),e.hasClass("with-timer")&&e.data("fecss-timer",setInterval(function(){e.is(":hover")||t.find(".btn-arrow.right").trigger("click.arrow-slider")},3e3)),a.find(".item.active").size()||a.find(".item").eq(0).trigger("click.arrow-slider")});
...
$(".line-gallery").each(function(e){var i=$(this);i.find(".btn-arrow.right").on("click.line-gallery.right",function(e){var t=i.find(".img-block .item"),n=t.filter(":visible");n.size()>1?(n.eq(0).hide().insertAfter(t.eq(-1)),n.eq(-1).next(".item").fadeIn("fast")):(n.eq(0).hide().insertAfter(t.eq(-1)),i.find(".img-block .item").eq(0).fadeIn("fast"))}),i.find(".btn-arrow.left").on("click.line-gallery.left",function(e){var t=i.find(".img-block .item"),n=t.filter(":visible");n.size()>1?(n.eq(-1).hide(),t.eq(-1).insertBefore(n.eq(0)).fadeIn("fast")):(n.eq(0).hide(),i.find(".img-block .item").eq(-1).insertBefore(i.find(".img-block .item").eq(0)).fadeIn("fast"))}),i.hasClass("with-timer")&&i.data("fecss-timer",setInterval(function(){i.is(":hover")||i.find(".btn-arrow.right").trigger("click")},3e3))});
...
$(function(){$(document.body).on("click.fecss",".rybalka-prud-photo-btns a",function(a){var t=$(this),r=t.attr("data-gal-id");$(".slider.rybalka").hide(),$('.slider.rybalka[data-gal-id="'+r+'"]').show()}),$(".rybalka-prud-photo-btns a").eq(0).trigger("click")});
